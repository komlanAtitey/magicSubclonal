magicSubclonal_GEO <- function(GEO_number, genes_of_interest, number_sample, gene_column_number) {
  ...
  # =====================================================
  # Step 1 — Load GEO dataset and Map driver genes (Robust version)
  # =====================================================
  ...
  # Helper: collapse rows with same gene symbol using median
  collapse_by_symbol <- function(mat) {
    ...
    out <- lapply(split_idx, function(ix) {
      if (length(ix) == 1L) mat[ix, , drop = FALSE]
      else matrixStats::colMedians(mat[ix, , drop = FALSE], na.rm = TRUE)
    })
  ...
  # Helper: map Ensembl IDs (with or without version) to gene symbols
  map_ensembl_to_symbol <- function(ids, keep_unmapped = TRUE) {
    stripped <- sub("\\..*$", "", ids)
  ...
  ################# Try loading processed GEO matrix
  cat(">>> Trying to load processed GEO ExpressionSet...\n")
  use_stmt1 <- FALSE
  ...
  if (use_stmt1) {
    ...
    ################# Case 1: Affymetrix GPL570 microarray
    cat(">>> Detected Affymetrix GPL570 platform. Mapping probes...\n")
    ...
    sym_map <- mapIds(hgu133plus2.db,
                      keys = rownames(exprs_data),
                      column = "SYMBOL",
                      keytype = "PROBEID",
                      multiVals = "first")
    rownames(exprs_data) <- sym_map
    exprs_data <- exprs_data[!is.na(rownames(exprs_data)), ]
    exprs_data <- collapse_by_symbol(exprs_data)
    
    cat(">>> Expression data loaded successfully from processed GEO matrix.\n")
    
  } else {
    
    ##################  Case 2: Try raw CEL file processing 
    cat(">>> Processed GEO matrix not found or incomplete. Trying CEL-based reconstruction...\n")
    ...
    tar_files <- list.files(supp_dir, pattern = "RAW\\.tar$", full.names = TRUE)
    
    if (length(tar_files) == 0) stop("No RAW tar file found for ", GEO_number)
    cel_dir <- file.path(supp_dir, "CEL")
    dir.create(cel_dir, showWarnings = FALSE)
    untar(tar_files[1], exdir = cel_dir)
    
    suppressPackageStartupMessages(library(affy))
    data_raw <- ReadAffy(celfile.path = cel_dir)
    data_norm <- rma(data_raw)
    exprs_data <- exprs(data_norm)
    ...
    cat(">>> Expression data reconstructed from CEL files.\n")
  }
  
  ################## Filter expression matrix for driver genes
  exprs_sub <- exprs_data[intersect(rownames(exprs_data), genes_of_interest), , drop = FALSE]
  
  cat(">>> Final dataset summary:\n")
  cat("   Genes:", nrow(exprs_sub), "\n")
  cat("   Samples:", ncol(exprs_sub), "\n")
  
  ################## Return objects
  list(
    expression_matrix = exprs_sub,
    full_expression   = exprs_data,
    phenotype_data    = pheno_data,
    probes_table      = if (exists("probes")) probes else NULL
  )
  ...
  # =====================================================
  # Step 3: Keep sample IDs for each resampled x0 (needed later)
  # =====================================================
  ...
  ################## Step 1: Download CEL files
  cel_dir <- file.path(tempdir(), GEO_number, "CEL")
  dir.create(cel_dir, recursive = TRUE, showWarnings = FALSE)
  
  # Download supplementary files
  getGEOSuppFiles(GEO_number, baseDir = cel_dir)
  
  # Extract CEL files from the raw tar archive
  raw_tar <- list.files(file.path(cel_dir, GEO_number), pattern = "_RAW.tar$", full.names = TRUE)
  if(length(raw_tar) > 0){
    untar(raw_tar[1], exdir = cel_dir)
  }
  
  ################## Step 2: Read CEL files and normalize
  ...
  data_norm <- rma(data_raw)  # Robust Multi-array Average normalization
  exprs_data <- exprs(data_norm)
  
  message("Normalized expression matrix: ", nrow(exprs_data), " features × ", ncol(exprs_data), " samples.")
  
  ################## Step 3: Map probes -> gene symbols
  probes <- AnnotationDbi::select(
    hgu133plus2.db,
    keys = rownames(exprs_data),
    columns = c("SYMBOL"),
  ...
  probes <- probes[!is.na(probes$SYMBOL), ]
  exprs_data <- exprs_data[probes$PROBEID, , drop = FALSE]
  rownames(exprs_data) <- probes$SYMBOL
  
  ################## Step 4: Collapse duplicate gene symbols
  collapse_by_symbol <- function(mat) {
    split_idx <- split(seq_len(nrow(mat)), rownames(mat))
    do.call(rbind, lapply(split_idx, function(ix) {
      if(length(ix) == 1L) mat[ix, , drop = FALSE] else {
        med <- matrixStats::colMedians(mat[ix, , drop = FALSE], na.rm = TRUE)
        matrix(med, nrow = 1, dimnames = list(rownames(mat)[ix][1], colnames(mat)))
      }
    }))
  }
  ...
  ################## Step 5: Subset non-driver genes
  non_driver_mat <- gene_mat[setdiff(rownames(gene_mat), genes_of_interest), , drop = FALSE]
  
  ################## Step 6: Output
  message("Gene matrix dimensions: ", paste(dim(gene_mat), collapse = " × "))
  message("Non-driver gene matrix dimensions: ", paste(dim(non_driver_mat), collapse = " × "))
  
  # gene_mat       # contains all genes
  # non_driver_mat # contains all genes except the driver genes
  ...
  # =====================================================
  # Step 12 Dynamic calibration for ALL genes + faceted plots
  # =====================================================
  sentence12 <- "Dynamic calibration via split simulations (all genes)!" # @@@@@@@@@@@@
  print(sentence12) # @@@@@@@@@@@@
  ...
  # =====================================================
  # Step 13: Biological plausibility checks (combined figure)
  # =====================================================
  sentence13 <- "Report t1/2 and steady-state moments; flag outliers + visualize!"  # @@@@@@@@@@@@
  print(sentence13)  # @@@@@@@@@@@@
  ...
  ################## Build plausibility table from fitted parameters -------------------
  ...
  ################## Half-life vs steady-state mean (log–log) ----------------------
  p14_scatter <- ggplot(
  ...
  ################## Variance–mean with reference curves ---------------------------
  p14_varmean <- ggplot() +
  ...
  plot_cme_overlays_with_filter <- function(gene_sims,
    ...
    ################## Standardize/repair filtered_df -------------------------------
    ...
    ################## subclone points (from Step 8 labels) -------------------------
    pts <- dplyr::bind_rows(lapply(genes, function(g) {
      df <- subclone_labs[[g]]
      if (!"gene" %in% names(df)) df$gene <- g
    ...
    ################## t_opt + rectangle around t_opt (auto width) ------------------
    ...
    ################## annotations: kept subclonal genes & counts -------------------
    ann <- filtered_df %>%
  ...
  # =====================================================
  # Step 14 (updated): Compute heterogeneity metrics per driver (new variables)
  # =====================================================
  sentence14 <- "Compute heterogeneity metrics per driver!"  # @@@@@@@@@@@@
  print(sentence14)  # @@@@@@@@@@@@
  ...
  always_heatmap_tumor_heterogeneity <- function(gene_mat,
    ...
    ################## Rare sample indices by driver; fallback to ALL samples
    ...
    ################## Build kept_by_driver with fallbacks (filtered -> assoc -> topvar per-driver)
    fsg <- NULL
    ...
    ################## Build matrix + COMPACT annotations (so legends don’t overlap) ----
    ...
    ################## Draw heatmap (compact, non-overlapping legends) ---
    p <- pheatmap::pheatmap(
      expr_scaled,
  ...
  # =====================================================
  sentence15 <- "Compute subclone likelihood & detect high-risk!"  # @@@@@@@@@@@@
  print(sentence15)  # @@@@@@@@@@@@
  ...
  ################## Build/standardize tumor_heterogeneity_df (robust) ----------
  ...
  ################## Visualization: high-risk at t_opt over CME envelopes ----------
